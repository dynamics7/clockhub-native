...
#define POWER_STATE_IDLE         (DWORD)(POWER_STATE_USERIDLE)  // idle state (same as useridle state)
...
#define POWER_SCREEN_UNKNOWN     (DWORD)(0x00000000)        // screen is in "unknown" state
#define POWER_SCREEN_ON          (DWORD)(0x00000001)        // screen is "ON"
#define POWER_SCREEN_OFF         (DWORD)(0x00000002)        // screen is "OFF"
...
#define POWER_SLOW_RESET        (DWORD)(0x00004000)     // slow reboot - notifies drivers
#define POWER_OOM_RESET         (DWORD)(0x00008000)     // flag set when device is rebooting under OOM conditions (kernel mode only)
...
typedef struct _POWER_CAPABILITIES {
    ...
    UCHAR WakeFromDx;
    UCHAR InrushDx;
    DWORD Power[PwrDeviceMaximum];     // Amount of power consumed at each device power state or POWER_CAPABILITIES_UNSPECIFIED
    DWORD Latency[PwrDeviceMaximum];   // Latency at each device power state
    DWORD Flags;
} POWER_CAPABILITIES, *PPOWER_CAPABILITIES;

// This value is used for Power  in the POWER_CAPABILITIES structure when the value isn't specified
#define      POWER_CAPABILITIES_UNSPECIFIED        0xFFFFFFFF
...
#define PBT_SCREENSTATUSCHANGE      0x00000010  // display turned on/off
#define PBT_POWERBATTERYFLAGCHANGE  0x00000020  // battery flag changed
...
typedef struct _POWER_BROADCAST {
    ...
    DWORD       Length;     // byte count of data starting at SystemPowerState
...
typedef struct _POWER_BROADCAST_POWER_STATUS_CHANGE  {
    ...
    DWORD       Flags;                  // Unused
    DWORD       Length;                 // Byte count of data starting at SystemPowerState (2)
...
// This structure is used instead of POWER_BROADCAST when the broadcast
// type is PBT_SCREENSTATUSCHANGE.
//
typedef struct _POWER_BROADCAST_SCREEN_STATUS_CHANGE  {
    DWORD       Message;                // PBT_SCREENSTATUSCHANGE 
    DWORD       Flags;                  // ununsed
    DWORD       Length;                 // Byte count of data starting at dwScreenStatus
    DWORD       ScreenFlags;            // screen on or off or in unknown state POWER_SCREEN_ON/OFF/UNKNOWN
} POWER_BROADCAST_SCREEN_STATUS_CHANGE, *PPOWER_BROADCAST_SCREEN_STATUS_CHANGE;

    
// This structure is used instead of POWER_BROADCAST when the broadcast
// type is PBT_POWERBATTERYFLAGCHANGE.
//
typedef struct _POWER_BROADCAST_POWER_BATTERYFLAG_CHANGE  {
    DWORD       Message;                // PBT_POWERSTATUSCHANGE 
    DWORD       Flags;                  // Unused
    DWORD       Length;                 // Byte count of data starting at SystemPowerState (2)
    WORD        NewBatteryFlag;         // bBatteryFlag
} POWER_BROADCAST_POWER_BATTERYFLAG_CHANGE, *PPOWER_BROADCAST_POWER_BATTERYFLAG_CHANGE;

// Static assert to make sure that POWER_BROADCAST_POWER_BATTERYFLAG_CHANGE matches POWER_BROADCAST.
C_ASSERT(FIELD_OFFSET(POWER_BROADCAST, SystemPowerState) == FIELD_OFFSET(POWER_BROADCAST_POWER_BATTERYFLAG_CHANGE, NewBatteryFlag));
...
BOOL ProximityPowerManagementIsEnabled(void);

BOOL ProximityPowerManagementEnable(
    BOOL fTurnOn
    );
